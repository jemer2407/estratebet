    ...
    for league in leagues:
        ...
        for td in td_a_match:
            ...
            if a != None:
                ...
                req_match = requests.get('{}{}'.format(url,a['href'].strip()), headers = headers).content.decode('utf-8')
                ...
                if soup_match.find('span',{'class':"jor-status jor-finished"}):
                    ...
                    if soup_match.find('span',{'class':"jor-status jor-finished"}).text == 'FINALIZADO':
                        ...
                        if match_obj.gol_home_ht == None:
                            ...
                            match_obj.goles_tramo_1_10 = goles_tramo_1_10
                            match_obj.goles_tramo_11_20 = goles_tramo_11_20                        
                            match_obj.goles_tramo_21_30 = goles_tramo_21_30                        
                            match_obj.goles_tramo_31_40 = goles_tramo_31_40                        
                            match_obj.goles_tramo_41_50 = goles_tramo_41_50
                            match_obj.goles_tramo_51_60 = goles_tramo_51_60
                            match_obj.goles_tramo_61_70 = goles_tramo_61_70
                            match_obj.goles_tramo_71_80 = goles_tramo_71_80
                            match_obj.goles_tramo_81_90 = goles_tramo_81_90
                            match_obj.goles_tramo_91_final = goles_tramo_91_final
                            match_obj.save()
                        else:
...
def create_teams(league,td_team):
    a_team = td_team.find_all('a')
    for team in a_team:                        
        ...
        if team.text != '':
            ...
            if response_img.status_code == 200:
                # guardamos la imagen
                ruta_completa = os.path.join(settings.MEDIA_ROOT, 'team/', "{}_img.jpg".format(team.text))
                with open(ruta_completa, "wb") as archivo:
                    archivo.write(response_img.content)

            print(team.text)
            team_format = limpiar_texto(team.text)
            print(team_format)
            team_obj = Team(team=team_format,league=league,image='team/{}_img.jpg'.format(team_format))
            team_obj.save()
            print('Equipo {} creado'.format(team_format))
...
def scraper_create_teams(request):
    ...
    for league in leagues:
        ...
        if not Team.objects.filter(league=league):
            url_league = '{}{}/grupo1/jornada1'.format(url,league.slug)
            print(url_league)
            req = requests.get(url_league, headers = headers).content.decode('utf-8')
            soup = BeautifulSoup(req, 'html.parser')
            etiq_table = soup.find('table', {'id': 'tabla1'})
            etiq_tr_team = etiq_table.find_all('tr', {'class': 'vevent'})
            
            for tr_team in etiq_tr_team:
                etiq_td_team1 = tr_team.find_all('td', {'class': 'equipo1'})
                etiq_td_team2 = tr_team.find_all('td', {'class': 'equipo2'})
                
                for td_team in etiq_td_team1:
                    create_teams(league,td_team)
                    
                for td_team in etiq_td_team2:
                    create_teams(league,td_team)
